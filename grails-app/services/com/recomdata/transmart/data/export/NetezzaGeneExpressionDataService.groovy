...
class NetezzaGeneExpressionDataService {
    ...
    public boolean getNzData(List studyList,
                             File studyDir,
                             String fileName,
                             String jobName,
                             String resultInstanceId,
        ...
        try {
            ...
            studyList.each { study ->
                ...
                if (null != resultInstanceId) {
                    ...
                    log.debug("mRNA Query : " + sqlQuery.toString())
                    log.debug("Sample Query : " + sampleQuery.toString())
                }

                // added to create clinical data view for nzr  by HX  01/10/2014
                if (subsetId.equals(null) || subsetId.size() == 0) {
                    ...
                    String tableName = jobName.replaceAll("-", "_") + "_MRNA" + "_" + subsetId.toUpperCase()
                    groovy.sql.Sql nzrSql = utilService.getNetezzaConnection()
                    if (!utilService.isTableExist(nzrSql, tableName)) {
                        nzrSql.execute("create table " + tableName + " as " + sqlQuery.toString(), [resultInstanceId])
                    }

                    String sampleTableName = jobName.replaceAll("-", "_") + "_MRNA_SAMPLE" + "_" + subsetId.toUpperCase()
                    if (!utilService.isTableExist(nzrSql, sampleTableName)) {
                        nzrSql.execute("create table " + sampleTableName + " as " + sampleQuery.toString(), [resultInstanceId])
                    }
                }
